...
function calcular() {
  ...
  // Validaciones
  if (c1 <= 0 || c7 < 0 || c8 < 0 || e2 <= 0 || !vin || !tipo || (tipo === 'TURISMO' && !esHibrido && !motor)) {
  ...
  guardarHistorial(
    detalles.map(([titulo, valor, unidad]) => ({ titulo, valor: unidad === 'usd' ? formatearUSD(valor) : formatear(valor) })),
...
function mostrarResultados(detalles) {
  const rows = detalles.map(([t, v, un]) => `
    <tr><td>${t}</td><td>${un === 'usd' ? formatearUSD(v) : formatear(v)}</td></tr>`).join('');
...
function descargarPDF() {
  ...
  w.document.write(`<html><head><title>PDF</title><style>body{font-family:Helvetica;margin:20px;} .tabla-detalles{border-collapse:collapse;width:100%;max-width:600px;margin:auto;} .tabla-detalles th,.tabla-detalles td{padding:8px 12px;border:1px solid #ddd;} .tabla-detalles th{background:#f2f2f2;}</style></head><body>${content}</body></html>`);
  w.document.close();
